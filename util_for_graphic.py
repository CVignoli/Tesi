...
class graphic_tools:
    ...
    def render3DMM(self, xIm, yIm, rgb, w, h):
        # need to perform the proj shape from the 3D model of the shape. The first column of projShape is xIm, the secondo is yIm.
        ...
        coords = self._2linear_vects(_first_cord.reshape(_first_cord.shape[0], 1),
                                     _second_cord.reshape(_second_cord.shape[0], 1))
    ...
    def calculateBarycentricInterpolationValues(self, sizep):
        ...
        return np.reshape(lambda1, (lambda1.shape[0], 1)), np.reshape(lambda2, (lambda2.shape[0], 1)), np.reshape(
            lambda3, (lambda3.shape[0], 1)), jind
